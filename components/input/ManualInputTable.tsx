...
export default function ManualInputTable({ colKey, colLabel, apiEndpoint, month, canEdit, saveMsg }: Props) {
  ...
  const details = lines.filter(l => !l.isGroup)
    ...
    <div style={{ padding: '16px 22px', overflow: 'auto', flex: 1 }}>
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>
          Preencha por fonte de recurso. O agrupamento visual segue a mesma hierarquia da planilha SIGEFES.
        </div>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead>
            <tr style={{ background: '#0F1624', color: 'white' }}>
              ...
              <th style={{ padding: '9px 14px', textAlign: 'center', fontSize: 11, background: 'rgba(234,179,8,0.2)', width: 200 }}>
            ...
            {groups.map(g => (
              <>
                <tr key={g.id} style={{ background: '#1e3a5f', color: 'white' }}>
                  <td colSpan={2} style={{ padding: '8px 14px', fontSize: 12, fontWeight: 500 }}>▸ {g.rowLabel}</td>
                </tr>
                {details.filter(d => d.id !== g.id).map((d, i) => (
                  <tr key={d.id} style={{ borderBottom: '0.5px solid var(--color-border-tertiary)', background: i % 2 === 0 ? 'var(--color-background-primary)' : 'var(--color-background-secondary)' }}>
                    <td style={{ padding: '7px 14px 7px 24px', color: 'var(--color-text-primary)', fontSize: 11, maxWidth: 280, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {d.rowLabel}
                    </td>
                    <td style={{ padding: '5px 10px', textAlign: 'center', background: '#fefce8' }}>
                      {canEdit ? (
                        <input
                          type="text"
                          value={vals[d.id] || ''}
                          ...
                          placeholder="0,00"
                          style={{ width: 150, padding: '5px 8px', fontSize: 12, border: '0.5px solid #fde047', borderRadius: 5, textAlign: 'right', background: 'white', color: 'var(--color-text-primary)', fontVariantNumeric: 'tabular-nums' }}
                        />
                      ) : (
